...
*
*  =====================================================================
      SUBROUTINE MYDPOTF2( UPLO, N, A, LDA, INFO )
*
...
*
     ...
     $            LDA )
            !IF( AJJ.LT.ZERO.OR.DISNAN( AJJ ) ) THEN
            IF (AJJ.LT.-EPS.OR.DISNAN( AJJ ) ) THEN
               A( J, J ) = AJJ
               GO TO 30
            END IF
*
*           If AJJ is nearly ZERO, threshold hardcoded.       
*
            IF ( AJJ.GT.ZERO) THEN
                AJJ = SQRT( AJJ )
                A( J, J ) = AJJ
